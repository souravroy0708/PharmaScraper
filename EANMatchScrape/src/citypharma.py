...
class citypharmaean(threading.Thread):
    ...
    def get_search_res(self):
        try:
            ...
            retdict=dict()
            retdict['url'] =  soup.find("h2",{"class":"product-name"}).find("a")['href']
            retdict['product']=soup.find("h2",{"class":"product-name"}).find("a").text.strip()
            retdict['site'] = self.config['site']
            retdict['image'] = soup.find("a",{"class":"product-image"}).find("img")['src']
