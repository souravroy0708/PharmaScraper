...
class PharmaplayProd:
    ...
    def get_allseg(self,soup):
        ...
        for anchor in soup.findAll('li', {"class":"parL2"}):
            ...
            segdict[anchor.text.strip().replace(anchor.find('span').text,"").strip()]['count']=int(anchor.find('span').text.replace(")","").replace("(",""))
            segdict[anchor.text.strip().replace(anchor.find('span').text,"").strip()]['url'] = self.config["site"]+anchor.find('a')['href']
        return segdict
    ...
    def get_allsubseg(self,soup):
        ...
        for anchor in soup.findAll('li', {"class":"parL3"}):
            ...
            subsegdict[anchor.text.strip().replace(anchor.find('span').text,"").strip()]['count']=int(anchor.find('span').text.replace(")","").replace("(",""))
            subsegdict[anchor.text.strip().replace(anchor.find('span').text,"").strip()]['url'] = self.config["site"]+anchor.find('a')['href']
        return subsegdict  
        ...
        while (self.is_product(url+"?&p="+str(pgid))):
            ...
            for prod in prods:
                try:
                    ...
                    proddict['Format']=prod.find("div",{"class":"caption"}).text.replace("\n","").replace(proddict['Product_name'],"").strip()
                    try:
                        proddict['urltoproduct']=self.config["site"] + prod.find("div",{"class":"caption"}).find("a")['href']
                    except Exception as e:
                        try:
                    ...
                    proddict['Imagefilename'] = prod.find("img",{"itemprop":"image"})['src'].split("/")[len(prod.find("img",{"itemprop":"image"})['src'].split("/"))-1]
                    try:
                        proddict['Price'] = float(prod.find("p",{"class":"special-price b-hideprice b-blurprice"}).text.replace("\n","").replace("\xa0€","").replace(",",".").strip())
                        proddict['Crossed_out_Price'] = float(prod.find("p",{"class":"old-price b-hideprice b-blurprice"}).text.replace("\n","").replace("\xa0€","").replace(",",".").strip())
                    except Exception as e:
                        self.logger.error("Line 118:"+str(e))
                        proddict['Price'] = float(prod.find("div",{"class":"price-box"}).text.replace("\n","").replace("\xa0€","").replace(",",".").strip())
                    ...
                    try:
                        if ("Marque" in prodsoup.find("div",{"itemtype":"http://schema.org/Product"}).find("p").text):
                            proddict["Brand"] = prodsoup.find("div",{"itemtype":"http://schema.org/Product"}).find("p").text.replace("Marque : ","").strip()
                        else:
                            proddict["Brand"] = "None"
                    except Exception as e: 
...
if __name__=="__main__":
    ...
    #config["mongolink"]="13.237.122.31:27017"
    sitedf = pd.read_csv(config['template']+"_sites.csv")
    for site in sitedf.Sites:
        break
        if (site[-1:]=="/"):
            config["site"]=site[:-1]
        else:
        ...
        config["urls"]=config["site"]+"/medicament/medicaments-p1"
        main(config)
        config['Mega-category']="parapharmacie"
        config["urls"]=config["site"]+"/parapharmacie"
        main(config)
