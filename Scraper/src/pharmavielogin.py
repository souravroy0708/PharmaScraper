...
class pharmavielogin(threading.Thread):
        ...
        while (run):
            ...
            for prod in prods:
                try:
                    ...
                    try:
                        proddict['urltoproduct'] = config['site'] + \
                                                   prod.find("h2", {"class": "product-title"}).find("a")['href']
                    ...
                    try:
                        proddict['Price'] = float(
                            prod.find("span", {"class": "price"}).text.replace("\n", "").replace("TTC", "").replace(
                                "\xa0€", "").replace(",", ".").strip())
                    except Exception as e:
                    ...
                    try:
                        ...
                        proddict["Brand"] = "None"
                    try:
                        proddict["Loyalty"] = prod.find("div",
                                                        {"class": "affinity-promotion affinity-fare"}).text.replace(
                            "\n", "").strip()
                    except Exception as e:
                        self.logger.error("Line 106:" + str(e))
                    ...
                    try:
                        proddict['Crossed_out_Price'] = float(
                            prod.find("s", {"class": "text-promo"}).text.replace("\n", "").replace("TTC", "").replace(
                                "\xa0€", "").replace(",", ".").strip())
                    except Exception as e:
                        self.logger.error("Line 133:" + str(e))
                        proddict['Crossed_out_Price'] = "None"
                    try:
                        proddict["Promotional_claim"] = prod.find("p", {
                            "class": "bg-promo p-2 mb-2 text-center text-promo text-uppercase"}).text.strip()
                    except Exception as e:
