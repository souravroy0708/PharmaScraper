...
class pharmabestprado1(threading.Thread):
        ...
        while (self.is_product(url + "PAGE=" + str(pgid))):
            ...
            for prod in prods:
                try:
                    ...
                    try:
                        proddict['Product_name'] = prod.find("h3").find("a")["title"].strip()
                        if (db['scrapes'].find(
                    ...
                    try:
                        proddict['urltoproduct'] = prod.find("h3").find("a")["href"].strip()
                    except Exception as e:
                    ...
                    try:
                        proddict['Availability'] = prodsoup.find("div", {"class": "availability_area"}).text.replace(" Disponibilité\xa0:", "").strip()
                    except Exception as e:
                        self.logger.error("Line 118:" + str(e))
                    ...
                    try:
                        ...
                        proddict['Availability'] = "None"
                    try:
                        proddict['Promotional_Claim'] = prod.find("div",{"class":"banner cart-rules"}).find("img")["title"]
                    except Exception as e:
